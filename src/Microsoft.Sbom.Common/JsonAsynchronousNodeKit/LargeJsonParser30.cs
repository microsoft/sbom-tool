...
/// </remarks>
public class LargeJsonParser30
{
    ...
    private readonly Dictionary<string, PropertyHandler> handlers;

    public LargeJsonParser30(
        Stream stream,
        Dictionary<string, PropertyHandler> handlers,
    ...
    private ParserStateResult HandleExplicitProperty(ref Utf8JsonReader reader, string propertyName)
    {
        ...
        switch (handler.Type)
        {
            ...
            case ParameterType.Object:
                ...
                result = this.GetObject(ref reader, objType);
                break;
    ...
    private object? ReadArrayObject(Type type)
    {
        try
        {
            ...
            {
                result = this.GetObject(ref reader, type);
            }
    ...
    private object GetObject(ref Utf8JsonReader reader, Type type)
    {
        var jsonObject = ParserUtils.ParseObject(this.stream, ref this.buffer, ref reader);
        var jsonObjectType = jsonObject.AsObject()["type"]?.ToString();
        object? result;

        if (jsonObjectType == null)
        {
            throw new InvalidOperationException($"Deserializing {jsonObjectType} object type is not supported.");
        }
        else
        {
            result = JsonConvert.DeserializeObject(jsonObject.ToString());
        }
