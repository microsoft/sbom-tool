...
{
    private readonly AsyncLocal<ConfigurationSetting<string>> buildDropPath = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> buildComponentPath = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> buildListFile = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> manifestDirPath = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> manifestPath = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> outputPath = new();
    private readonly AsyncLocal<ConfigurationSetting<int>> parallelism = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> configFilePath = new();
    private readonly AsyncLocal<ConfigurationSetting<IList<ManifestInfo>>> manifestInfo = new();
    private readonly AsyncLocal<ConfigurationSetting<AlgorithmName>> hashAlgorithm = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> rootFilterPath = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> catalogFilePath = new();
    private readonly AsyncLocal<ConfigurationSetting<bool>> validateSignature = new();
    private readonly AsyncLocal<ConfigurationSetting<bool>> ignoreMissing = new();
    private readonly AsyncLocal<ManifestToolActions> manifestToolAction = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> packageName = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> packageVersion = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> packageSupplier = new();
    private readonly AsyncLocal<ConfigurationSetting<IEnumerable<SbomFile>>> filesList = new();
    private readonly AsyncLocal<ConfigurationSetting<IEnumerable<SbomPackage>>> packagesList = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> telemetryFilePath = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> dockerImagesToScan = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> externalDocumentReferenceListFile = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> additionalComponentDetectorArgs = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> namespaceUriUniquePart = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> namespaceUriBase = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> generationTimestamp = new();
    private readonly AsyncLocal<ConfigurationSetting<bool>> followSymlinks = new();
    private readonly AsyncLocal<ConfigurationSetting<bool>> fetchLicenseInformation = new();
    private readonly AsyncLocal<ConfigurationSetting<bool>> enablePackageMetadataParsing = new();
    private readonly AsyncLocal<ConfigurationSetting<bool>> deleteManifestDirIfPresent = new();
    private readonly AsyncLocal<ConfigurationSetting<bool>> failIfNoPackages = new();
    private readonly AsyncLocal<ConfigurationSetting<LogEventLevel>> verbosity = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> sbomPath = new();
    private readonly AsyncLocal<ConfigurationSetting<string>> sbomDir = new();
