        ...
        Task.Run(async () =>
        {
            await foreach (var externalDocumentReferenceInfo in externalDocumentReferenceInfos.ReadAllAsync())
            {
                foreach (var config in externalDocumentReferenceArraySupportingConfigs)
                {
                    ...
                    {
                        log.Warning("Encountered an error while generating json for external document reference {Name}: {Message}", externalDocumentReferenceInfo.ExternalDocumentName, e.Message);
