...
public class SbomValidationWorkflowBase
{
    ...
    public async Task<bool> RunAsync(ISbomConfig sbomConfig, string eventName, ConformanceType conformanceType, bool validateSignature, bool failIfNoPackages, bool ignoreMissing)
    {
            ...
            try
            {
                ...
                if (validateSignature)
                {
                    ...
                    else
                    {
                        ...
                        RecordAdditionalTelemetry(additionalTelemetry); // TODO make this specific to the event name
                        if (!signatureIsValid)
                        {
